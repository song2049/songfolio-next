...
const CASES: CompanyCase[] = [
{
  ...
  role: "IT / 자산관리담당자",
  outcome:
    "약 50종 라이선스와 25개 벤더 운영을 2023년 2.5억 예산 기준으로 정리하고, 갱신 리스크(약 7천만)를 관리 가능한 구조로 표준화",
  problem: [
    "부서/개인별 구매로 기준이 불명확",
  ...
  structure: [
    "관리자(Admin) 구매 원칙 및 라이선스 운영 프로세스 정립",
    "권한(RBAC)·변경이력(Audit Log) 기반 운영 기준 설계",
    "상태/주기/소유/할당을 데이터 구조로 고정",
    "매뉴얼 제작 및 전사 공유",
  ],
  result: [
    "약 50종 라이선스·25개 벤더·연간 2.5억 예산 규모 운영 체계 안정화",
    "갱신/반납/만료 누락 감소로 비용 및 운영 리스크 완화",
